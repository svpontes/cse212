...
public static class Search
{
    ...
    private static int SearchSorted1(int[] data, int target)//linear search. Count comparation was made sequential (linear)  Complexidy: O(n). Cresce proporcionalmente ao tamanho da lista
    {
        var count = 0;
    ...
    private static int SearchSorted2(int[] data, int target, int start, int end)//binary recursive search. Complexity: O(log n) . Cresce mais lentamente
    {
        if (end < start)
            return 1; // All done
